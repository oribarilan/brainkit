...
- [ ] **Context size indicator** — Show token/context usage in status bar so the user knows how much vault context is being injected. Help avoid hitting context limits in long sessions.
...
- [ ] **Recurring reminders** — "Remind me to update my bragfile every Friday." Agent checks on session start and nudges if overdue.
...
- [ ] **Test suite** — Unit tests for vault operations, bragfile parsing, contacts parsing, system prompt builder, changelog parser, version comparison.
