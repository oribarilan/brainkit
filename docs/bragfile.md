...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Adding entries        | Agent uses built-in file editing, guided by the bragfile skill for format and placement rules                                                            | Agent uses built-in file editing, guided by bragfile skill in `~/.config/brainkit/copilot/skills/brainkit/references/bragfile.md`        |
...
| Staleness reminders   | System prompt injection via `experimental.chat.system.transform` — `buildBragReminder()` checks last entry date and adds reminder text if 14+ days stale | Static in `~/.config/brainkit/copilot/copilot-instructions.md`, generated at launch with current staleness data; not updated mid-session |
