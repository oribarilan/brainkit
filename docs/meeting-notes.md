...
| Placement decision        | Agent applies meeting-notes skill knowledge to pick the right PARA directory; system prompt injects current vault structure and, if the user's working directory matches a project, that project's context | Agent applies skill knowledge; `~/.config/brainkit/copilot/copilot-instructions.md` provides vault structure (static at launch) |
